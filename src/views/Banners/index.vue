...
export default {
  ...
  computed: {
    ...mapGetters({
      'tableData': 'banner/getBanners',
      'total': 'banner/getTotal'
    }),
    key() {
  ...
  methods: {
    ...
    async list() {
      await this.fetchListBannerByPaginate(this.listQuery);
      this.isTabTrashed = false;
    },
    async trashed() {
      await this.fetchListBannerTrashedByPaginate(this.listQuery);
